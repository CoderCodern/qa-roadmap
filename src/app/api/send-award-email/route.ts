...
export async function POST(req: NextRequest) {
  ...
  const { productName, productNameVi, price, points, type, phaseId } = body
  if (!productName || !type) {
  ...
  try {
    const { error } = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL ?? 'QA Roadmap <onboarding@resend.dev>',
      to: adminEmail,
      subject,
      html: buildHtml({ productName, productNameVi, price, points, type, phaseId }),
    })
